...
create type house_type_enum as enum('House','Apartment');
...
alter table owners add constraint unique_email  unique(owner_email) ;
...
alter table owners rename column owner_email to email;
...
where owner_id=(select max(length(first_name))from owners);
...
select  owners.first_name as owner_name,concat(addresses.city,' ',addresses.region,' ',addresses.street) as address, houses.* from houses
                                                                                                                                       join addresses  on addresses.id = houses.address_id
...
--         where rating =(select min(rating)) or rating =(select avg(rating));

-- - 13.Уйлору жок киленттерди, клиенттери жок уйлорду чыгар.
select * from customers,houses
                            join rent_info  on houses.id = rent_info.house_id
                            join rent_info on rent_info.customer_id=customers.customer_id;
...
select owners.first_name,customers.first_name from owners full join customers on  customers.id=owners.id
where owners.first_name like 'A%' ,
